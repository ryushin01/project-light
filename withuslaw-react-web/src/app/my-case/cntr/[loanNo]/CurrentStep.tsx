...
const DDay = ({
  ...
  const { seller, buyer, repayList, adminReqStatCd } = usePayInfoData({
    loanNo,
  });
  ...
  const hrefExtractor = () => {
    ...
    // 서류 제출 프로세스 진행 > 대출 취급 주체 코드: 은행지점(1) + 상태 코드: 대출 실행(10) 또는 인증 확인 중(11) + 서류 제출 프로세스 진행 중: 서류 요청 중(00) 또는 서류 확인 중(01) 또는 서류 반려(02)
    if (
      props.lndHndgSlfDsc === "1" &&
      (statCd === "10" || statCd === "11") &&
      (adminReqStatCd === "00" ||
    ...
    // 대출금 처리 > 대출 취급 주체 코드: 은행지점(1) + 상태 코드: 대출 실행(10) 또는 인증 확인 중(11) + 서류 제출 프로세스 미진행 초기 상태
    if (
      props.lndHndgSlfDsc === "1" &&
      (statCd === "10" || statCd === "11") &&
      adminReqStatCd === ""
    ...
    /* NOTE : 2025.05.29 SR 진행 이전 SR 케이스 분기 주석처리 */
    /* TODO : 6월 20일에 (SR오픈일에 맞춰서) 주석 해제 */
    /* 주석 여기서 부터 */
    // sr건이고, 대출금 처리
    // if (
  ...
  }, [progGbInfo, statCd, adminReqStatCd]);

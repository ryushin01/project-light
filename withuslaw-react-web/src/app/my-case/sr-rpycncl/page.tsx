...
export default function SR_RPYCNCL() {
  ...
  // 대출금(상환금) 지급 결과 정보 조회
  const { data: a400Response } = useQuery({
    queryKey: ["a400ResponseCode", loanNo],
    queryFn: () =>
      fetchApi({
        url: `${process.env.NEXT_PUBLIC_APP_WOORI_API_URL}/api/reapay/searcha400receiveyn?loanNo=${loanNo}`,
        method: "get",
      })
        .then((res) => res.json())
        .then((res) => res.data),
  ...
  const { mutate: payRefundRequest, data: refundData } = useMutation({
    ...
    onSuccess(data, variables, context) {
      ...
      if (data.code === "00") {
        ...
        callToast({
          msg: "대출금 요청이 실패하였습니다. 다시 시도해 주세요.",
          status: "error",
        });
      }
    },
    ...
    <>
      ...
      <div className="pt-5 pb-24">
        ...
        <section className="flex flex-col w-full divide-y">
          ...
          >
            {/* 대출금 지급 진행 중 + 괸리자 요청 전 */}
            {a400Response?.resCd === "N" &&
              srRepayInfo?.adminReqStatCd === "00" && (
                <div className="flex w-full justify-between">
                  <PayProceeding text={"대출금 지급처리 중"} />
                  <RpyCnclResult
                    a400ResCd={a400Response?.resCd}
            ...
            {/* 대출금 지급 실패 */}
            {a400Response?.resCd === "E" && (
              <div className="flex justify-between">
                <PayFail errCd={a400Response?.errMsg} />
                <Button.CtaButton
                  size={Size.Small}
                  state={"Default"}
                  onClick={() =>

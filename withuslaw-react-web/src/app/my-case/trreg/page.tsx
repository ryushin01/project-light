...
export default function MY_TR_001_M() {
  ...
  const { data: initRegInfo, isLoading: regInfoLoading } = useQuery({
  ...
  const { mutate: checkTrreg } = useMutation({
    ...
    onSuccess(data, variables, context) {
      const falseCase = data.data.filter((el: any) => el.available === false);

      if (falseCase.length > 0) {
        callToast({
          msg: "등기신청 사건이 존재하지않습니다.\n입력내용을 확인해주세요.",
          status: "error",
          // dim: true,
        });
        /* 불일치 접수번호 노출 임시 주석 */
        // const message = falseCase
  ...
  const { mutate: saveAcptnoReg } = useMutation({
    ...
    onSuccess(data, variables, context) {
      ...
      if (data.code === "00") {
        ...
        // 에러 케이스(data.code !== "00"): 바텀 시트 닫기 > 토스트 팝업 노출
        closeConfirm();
  ...
  const onSubmit = (data: TForm) => {
    const onlyNumValue = data?.faRgstrUnqNo1?.replace(/\D/g, "").slice(0, 14); // 특수문자 제외한 숫자만 유지
    ...
    if (regType === RGSTR_GB_CD["01"]) {
      ...
      //   loanNo, // 여신번호
      //   tempUniqNo: onlyNumValue!, // 등기고유번호
      // }));
      // //payload 저장
      // setPayload({
      //   checkList,
  ...
  */
  ...
  if (regInfoLoading) return <Loading />;
